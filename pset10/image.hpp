...
class Image
{
  ...
  public:
    ...
    int width() const
    {
        return (*this).width_; // IHR CODE HIER
    }

        // Hoehe abfragen
    int height() const
    {
        return (*this).height_; // IHR CODE HIER
    }

        // Gesamtzahl der Pixel abfragen
    int size() const
    {
        return (*this).height_*(*this).width_; // IHR CODE HIER
    }

        // Groesse der Bildes aendern
    void resize(unsigned int new_width, unsigned int new_height)
    {
        (*this).width_ = new_width;// IHR CODE HIER
        (*this).height_ = new_height;
        (*this).data_.resize(new_width*new_height, uint16_t());
    }

        // lesender Zugriff auf des Pixel an Position (x,y)
    PixelType operator()(int x, int y) const
    {
        return data_[x + y * width_]; // IHR CODE HIER
    }
...
{
    // IHR CODE HIER
    if(im0.height() != im1.height() || im0.width() != im1.width())
        return false;

    for(int y=0, h=im0.height(); y<h; ++y)
    {
        for(int x = 0, w=im0.width(); x<w; ++x)
        {
            if(im0(x,y) != im1(x,y))
                return false;
        }
...
std::string to_string(Image const & im)
{
    std::string res;
    // IHR CODE HIER
    for(int j = 0, h=im.height(); j < h; ++j)
    {
        for(int i = 0, w=im.width(); i<w; ++i)
        {
